...
export class RatingComponent implements ControlValueAccessor, OnInit, OnChanges {
  ...
  /**
   * The size of start element. integer value like `3`.
   */
  @Input()
  size = 5;
  ...
  constructor() {
    const config = inject(RatingService);
  ...
  private _updateState(nextValue: number) {
    ...
    this.contexts.forEach(
      (context, index) => (context.fill = Math.round(this._getValueInRange(nextValue - index, 1, 0) * 100)),
    );
  }
